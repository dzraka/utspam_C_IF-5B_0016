...
class RentFormPage extends StatefulWidget {
  final Car car;
  final User user;
  const RentFormPage({super.key, required this.car, required this.user});
...
class _RentFormPageState extends State<RentFormPage> {
  ...
  void _submit() async {
    if (_formKey.currentState!.validate()) {
      final rentDays = int.parse(_daysCtr.text);

      final transaction = RentTransaction(
      ...
      final result = await _repo.insertTransaction(transaction);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text("Formulir Sewa")),
